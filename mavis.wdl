...
task generateConfigScript {
  ...
  parameter_meta {
    ...
    arribaConverter: "Path to arriva converter script"
...
task config {
  ...
  parameter_meta {
    ...
    masking: "Path to reference masking file"
    dgvAnnotations: "Path to reference Database of Genome Variants annotations"
    alignerReference: "Path to reference for the Mavis aligner"
    templateMetadata: "Path to Mavis template metadata file"
    jobMemory: "Memory for the task, in gigabytes"
    modules: "Environment modules for the task"
    timeout: "Timeout for the task, in hours"
  }
...
task setup {
  ...
  command <<<
    ...
    # Get the batch ID from the "build.cfg" file generated by setup
