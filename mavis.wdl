...
workflow mavis {
  ...
  scatter(ct in runMavisSetup.clusterTabs) {
    call runMavisSubmit as validate {
      input: inFiles=ct, script=runMavisSetup.submitValidate, outFileName="validation-passed.tab"
    }
    call runMavisSubmit as annotate {
      input: inFiles=validate.outFile, script=runMavisSetup.submitAnnotate, outFileName="annotations.tab"
    }
  }
  call runMavisPairing {
    input: inFiles=annotate.outFile, script=runMavisSetup.submitPairing
  }
...
task runMavisSubmit {
  ...
  # edit and run a submit.sh script generated by Mavis
...
task runMavisPairing {

  # edit and run a pairing submit.sh script generated by Mavis
  ...
  input {
    ...
    Int jobMemory = 12
    Int timeout = 24
    String outDirName = "pairing"
  }

  # order of --inputs and --output is inconsistent; always follow with a \

  # String inFileString="~{sep=' ' inFiles}"
  ...
  output {
    Array[File] paired = glob("~{outDirName}/mavis_paired*.tab")
  }
